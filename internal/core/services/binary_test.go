...
func TestBinaryService_Delete_SecretRepoErr(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockSecretRepo.EXPECT().Delete(gomock.Any(), "file1", int64(1)).Return(errors.New("db error"))

	err := service.Delete(context.Background(), 1, "file1")
	assert.Error(t, err)
}

func TestBinaryService_Delete_BinaryRepoErrLogged(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockSecretRepo.EXPECT().Delete(gomock.Any(), "file1", int64(1)).Return(nil)
	mockRepo.EXPECT().Delete(gomock.Any(), "1/file1").Return(errors.New("storage error"))

	err := service.Delete(context.Background(), 1, "file1")
	assert.NoError(t, err)
}

func TestBinaryService_EnsureBinarySecret_NotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockSecretRepo.EXPECT().GetByID(gomock.Any(), "file1", int64(1)).Return(nil, domain.ErrSecretNotFound)

	err := service.Delete(context.Background(), 1, "file1")
	assert.ErrorIs(t, err, domain.ErrSecretNotFound)
}

func TestBinaryService_EnsureBinarySecret_NotBinaryType(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	err := service.Delete(context.Background(), 1, "file1")
	assert.ErrorIs(t, err, domain.ErrBinarySecretRequired)
}
